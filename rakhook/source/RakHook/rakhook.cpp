...
kthook::kthook_simple<destroy_ri_t> destroy_ri_;
kthook::kthook_simple<handle_rpc_packet_t> handle_rpc_packet_;


// callbacks
void destroy_rakclient_interface(const decltype(destroy_ri_) &hook, void* rakclient_interface) {
...
bool __fastcall handle_rpc_packet(const decltype(handle_rpc_packet_)& hook, void *rp, void *edx, const char *data,
...
bool rakhook::initialize() {
    ...
    *rakclient_interface = std::bit_cast<RakClientInterface*>(hooked_interface);
    ...
    destroy_ri_.set_dest(samp_addr(offsets::destroy_interface(false)));
    destroy_ri_.set_cb(&destroy_rakclient_interface);
    destroy_ri_.install();
    
    handle_rpc_packet_.set_dest(samp_addr(offsets::handle_rpc_packet(false)));
    handle_rpc_packet_.set_cb(&handle_rpc_packet);
    handle_rpc_packet_.install();
...
bool rakhook::emul_rpc(unsigned char id, RakNet::BitStream& rpc_bs) {
    ...
    if (handle_rpc_packet_.get_trampoline()) {

        handle_rpc =
            std::bit_cast<handle_rpc_packet_original_t>(
            handle_rpc_packet_.get_trampoline());
